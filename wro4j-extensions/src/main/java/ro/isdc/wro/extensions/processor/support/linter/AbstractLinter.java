...
public abstract class AbstractLinter {
    ...
    /**
     * @return the stream of the linter script. Override this method to provide a
     *         different script version.
     * @throws IOException
    ...
    /**
     * Validates a js using jsHint and throws {@link LinterException} if the js is
     * invalid. If no exception is thrown, the
     * js is valid.
     *
     * @param data
    ...
    public void validate(final String data)
        ...
        LOG.debug("[DEBUG] Validando JS que inicia con: "
                + data.substring(0, Math.min(40, data.length())).replace("\n", " ") + "...");
        ...
        if (!packIt.contains(",{});")) { // Solo imprime si hay opciones
            LOG.debug("[DEBUG] Script executed in JSHint: " + packIt);
        }
        // Execute JShint.
        ...
        if (!valid) {
            final String json = builder.addJSON().evaluate(
                    String.format("JSON.stringify(JSON.decycle(%s.errors))", getLinterName()), "stringify errors")
                    .toString();
            LOG.debug("json {}", json);
            final Type type = new TypeToken<List<LinterError>>() {
            }.getType();
            final List<LinterError> errors = new Gson().fromJson(json, type);
            LOG.debug("[DEBUG] Errores JSHint:");
            if (errors != null) {
                for (LinterError err : errors) {
                    if (err != null) {
                        LOG.debug(String.format("  - Line %d, Char %d: %s%n    Evidence: %s%n",
                                err.getLine(), err.getCharacter(), err.getReason(), err.getEvidence()));
                    }
                }
            }
