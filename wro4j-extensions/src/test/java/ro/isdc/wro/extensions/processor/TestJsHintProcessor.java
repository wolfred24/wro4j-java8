...
public class TestJsHintProcessor
    ...
    @Test
    public void testGlobalsOptionIsParsedCorrectly() throws Exception {
        final ThreadLocal<Throwable> cause = new ThreadLocal<Throwable>();
        // Usa una variable global que normalmente marcaría error
        String js = "dojo = 1;\ndefine = function(){};\nale rt(1);\nalert(;";
        final ResourcePostProcessor processor = new JsHintProcessor() {
            @Override
            protected void onLinterException(final LinterException e, final Resource resource) {
        ...
        }.setOptionsAsString("esnext=true,undef=true,predef=['dojo','define']");

        processor.process(new StringReader(js), new StringWriter());
        Assert.assertNotNull(cause.get());
    }

    /**
     * This test was created initially to prove that {@link JsHintProcessor} is
     * thread-safe, but it doesn't work well when
